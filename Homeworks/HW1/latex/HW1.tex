...
\end{equation}
The full C++ implementation is displayed below (raw output is provided in Appendix A):
\newpage
...
The trajectory of a single random walker (Task a) is shown in Figure \ref{fig:random_walk}. The path exhibits characteristic diffusive behavior, moving away from the origin in a jagged, non-linear path.
...
\section{Raw Terminal Output (Problem 2)}
Below is the exact, unedited terminal output from the Monte Carlo simulation (Problem 2).

\begin{outputbox}{Raw Terminal Output: Problem 2 Results}
    ==========================
